...
const list = document.querySelector('#list'); // Unordered list container

// Load saved chapters from localStorage on page load
document.addEventListener('DOMContentLoaded', function () {
    const savedChapters = JSON.parse(localStorage.getItem('chapters')); // Retrieve saved chapters
    if (savedChapters) {
        savedChapters.forEach(chapter => addChapterToList(chapter)); // Add each chapter to the list
    }
});
...
button.addEventListener('click', function (event) {
    ...
    const isValidFormat = /^[a-zA-Z]+\s+\d+$/.test(chapter); // Regex to validate format
    if (!isValidFormat) {
...
function addChapterToList(chapter) {
    ...
    deleteButton.setAttribute('aria-label', `Remove ${chapter}`); // Accessibility attribute
...
function saveChaptersToLocalStorage() {
    const chapters = Array.from(list.children).map(li => li.querySelector('span').textContent); // Extract chapter text
    localStorage.setItem('chapters', JSON.stringify(chapters)); // Save to localStorage
}
